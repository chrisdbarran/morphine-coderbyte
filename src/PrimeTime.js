...
Also since for example 256 x 256 = 65536, one of the factors will always 
be less than the square root of the number under test so we only need to 
test primes upto the sqrt of the number under test.

So the solution here is first to calculate all the primes up to the 
square root of the number under test. Then see if any of these primes 
divides equally into the number under test. If it does it's not a prime.

I'm using the every function in JS to check that every prime in the primes 
